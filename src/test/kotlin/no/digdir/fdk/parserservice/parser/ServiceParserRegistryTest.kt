...
/**
 * Test class for ServiceParserRegistry.
 */
@Tag("unit")
class ServiceParserRegistryTest {
    @Test
    fun `should register parsers and execute in priority order`() {
        val registry = ServiceParserRegistry()
    ...
    fun `should parse with all parsers and return results in priority order`() {
        ...
        val results = registry.parseWithAllParsers(model, "http://example.org/service", "test-id")
    ...
    fun `should throw exception when no parsers succeed`() {
        ...
        assertThrows<IllegalStateException> {
            registry.parseWithAllParsers(model, "http://example.org/service", "test-id")
        }
    }
