...
/**
 * Test class for ConceptParserRegistry.
 */
@Tag("unit")
class ConceptParserRegistryTest {
    ...
    fun `should register parsers and execute in priority order`() {
        val registry = ConceptParserRegistry()
    ...
    fun `should parse with all parsers and return results in priority order`() {
        ...
        val results = registry.parseWithAllParsers(model, "http://example.org/concept", "test-id")
    ...
    fun `should throw exception when no parsers succeed`() {
        ...
        assertThrows<IllegalStateException> {
            registry.parseWithAllParsers(model, "http://example.org/concept", "test-id")
        }
    }
    ...
    fun `results are returned in parser priority order`() {
        ...
        val model = ModelFactory.createDefaultModel()
        val results = registry.parseWithAllParsers(model, "http://example.org/concept", "fdk-id")
