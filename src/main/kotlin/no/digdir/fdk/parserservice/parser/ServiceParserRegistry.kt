    ...
    fun parseWithAllParsers(model: Model, iri: String, fdkId: String): List<Service> {
        ...
        for (entry in parsers) {
            try {
                ...
                results.add(service)
                ParseMetrics.recordProfileMatch(RdfParseResourceType.SERVICE, entry.name, matched = true)
                LOGGER.debug("Successfully parsed service with parser '${entry.name}'")
            } catch (e: Exception) {
                ParseMetrics.recordProfileMatch(RdfParseResourceType.SERVICE, entry.name, matched = false)
